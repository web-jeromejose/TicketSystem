...
namespace TicketSystem.Areas.Identity.Pages.Account.Manage
{
    public class EnableAuthenticatorModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            ...
            await LoadSharedKeyAndQrCodeUriAsync((ApplicationUser)user);
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            await _userManager.SetTwoFactorEnabledAsync((ApplicationUser)user, true);
            var userId = await _userManager.GetUserIdAsync((ApplicationUser)user);
            _logger.LogInformation("User with ID '{UserId}' has enabled 2FA with an authenticator app.", userId);

            StatusMessage = "Your authenticator app has been verified.";

            if (await _userManager.CountRecoveryCodesAsync((ApplicationUser)user) == 0)
            {
                var recoveryCodes = await _userManager.GenerateNewTwoFactorRecoveryCodesAsync((ApplicationUser)user, 10);
